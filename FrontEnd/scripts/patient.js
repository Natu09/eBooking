...
$(document).ready(async function () {
  ...
  deleteApt = function (event, element, view) {
    ...
    $('#cancelAppointment').on('click', async function () {
      $('#cancelAppointment').unbind()
      const apt = {
...
function getAllDoc(id) {
  return $.get(`${AUTH_URL}/user/doc/${id}`)           // Need to change this
}

function getAllAvailableApt(id) {
  return $.get(`${AUTH_URL}/user/availabilities/${id}`)      // Need to change this
}

function getAllApt(id) {
  return $.get(`${AUTH_URL}/user/appointments/${id}`)
}
...
async function getAllEvents(id) {
  try {
    ...
    let booked = await getAllApt(id)
    let result = available.concat(booked)
    console.log(result)
    return result
  } catch (error) {
    console.log(error)      // Try to make error a pop up message
  }
